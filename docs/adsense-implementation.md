...
リクエスト URL（`googleads.g.doubleclick.net/pagead/ads?client=...&format=250x250&url=...`）が正しく組み立てられているのに 400 になる場合、**実装ではなく AdSense 側の設定**が原因であることが多いです。

### 1. サイト（ドメイン）を AdSense に追加しているか（最重要）

広告を表示したいドメイン（例: `adgame-web.skikatte.com`）が、**AdSense の「サイト」一覧に追加されていないと、広告リクエストが 400 や未配信になる**ことがあります。

- AdSense 管理画面 → **「サイト」**（または「アカウント」→「サイトの管理」）を開く。
- 表示したい **完全なドメイン**（例: `https://adgame-web.skikatte.com`）が一覧にあり、**「準備完了」** または「承認済み」になっているか確認する。
- まだ追加していない場合: 「サイトを追加」から該当ドメインを追加し、表示される手順で**所有権確認**を行う（HTML タグや DNS など）。
- 追加直後は「準備中」「審査中」となり、**審査が通るまで広告が配信されない**ことがある（目安: 数日〜2〜4 週間）。
...
- 開発時は StrictMode により 2 回マウントされ、`push` が 2 回走ることがある。本番では 1 回になる。本番ビルドでも 400 が出るか確認する。
